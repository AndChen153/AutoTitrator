...
GPIO.setup(mot1_Dir, GPIO.HIGH)
p1_channel = 3      # HIGH = inject; LOW = fill
# Motor 2
mot2_step = 11
mot2_Dir = 13
...
progress_mot2.place(x=50, y=200)
p1bar_label = Label(root, text="P1", bg="RoyalBlue1", foreground="white", font=myfont)
p1bar_label.place(x=15, y=305)
p2bar_label = Label(root, text="P2", bg="RoyalBlue1", foreground="white", font=myfont)
...
report_mainframe = Frame(container, bg="RoyalBlue1")
...
report_mainframe.place(in_=container, x=0, y=0, relwidth=1, relheight=1)
THM_mainframe.lift()
#progress_frame.lift()
...
report_button = Button(tool_frame, text="RESULT-LOG", command=pagefour, font=myfont)
report_button.pack(side=LEFT, padx=2, pady=2)
...
def volume_check(dispense_volume, check = True):
	distance = pot.read_adc_difference(p2_channel, gain=GAIN)
	calib_volume = (distance - 7380.59) / 3205.8
...
class Titration_loop:
	...
	def Read_pH(self):
		...
		f_pH = float(pH)
		pH_label.config(text='{0:.4} pH/{1}{2}'.format(pH, current_temp,(u"\u2103").encode('utf-8')))
		return f_pH
	...
	def Refill(self):
		...
		valve_control('Alkalinity', 'ON')
		distance = pot.read_adc_difference(p2_channel, gain=GAIN)
		calib_volume = 	float((distance - 7380.59) / 3205.87)
	...
	def Colorimetry(self):
		...
		new_RGB = spectra.readRawValues()
		RGB.append(new_RGB)
		if RGB[len(RGB)-1][5]>0 and new_RGB[5] < 0:	
			Titration_loop.cnt =+ 1			
			step_volume = 0.5
			if self.read_volume(step_volume) != False:
				self.Dispense_step_volume(step_volume)
				self.initiate()
				...
				if fork == 1:
					...
					if self.check_stop() == False:	
						if Titration_loop.rf == True:
							...
							Titration_loop.rf = False
							#valve_control('Hardness', 'ON')
							self.Dispense_step_volume_p2(0.25)
							time.sleep(10)
							r2.get(False)
							root.after_cancel(barupdate_id_p2)
							#if tkMessageBox.askyesno('Ready', 'Refill Completed.\n''\n' 'Place the analyte beaker back under the dispensing tube.'): 
...
def get_baseline(type_of_titration):
	if type_of_titration == 'General':
		Titration.Colorimetry()
	elif type_of_titration == 'Hardness':
		spectra.startMeasurement()
		rdy = False
		while not rdy:
			time.sleep(.005)
...
class Create_table:
	def __init__(self):
		report_subframe = Frame(report_mainframe)
		report_subframe.pack(fill=BOTH,expand=1)
		...
		self.table = pt = Table(report_subframe, dataframe=df.sort_values(by=['Date/Time(H)'], ascending=False),
	...
	def update(self):
		Alkalinity = pd.read_csv('/home/pi/Dispenser_gui/Alkalinity_Result_log.csv')
		Hardness = pd.read_csv('/home/pi/Dispenser_gui/Hardness_Result_log.csv')
		df = pd.concat([Hardness,Alkalinity], axis=1).sort_values(by='Date/Time(H)', ascending=False)
...
def sys_check():
	...
	time.sleep(3)
	try:
		pump1 = pot.read_adc_difference(p2_channel, gain=GAIN)
		calib_volume1 = (pump1 - 7380.59) / 3205.8
		if not -0.3 < calib_volume1 < 5.1:
			...
			ready = False
	try:
		pump2 = pot.read_adc_difference(p1_channel, gain=GAIN)
